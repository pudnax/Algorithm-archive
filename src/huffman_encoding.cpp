...
public:
  template <typename Iter> codebook(Iter const first, Iter const last);

  template <typename Iter>
  std::vector<bool> encode(Iter first, Iter last) const;

  template <typename Iter> std::string decode(Iter first, Iter last) const;
};
...
template <typename Iter> codebook::codebook(Iter const first, Iter const last) {
  struct helper {
    static node_ptr make_decoder(Iter const first, Iter const last) {
      ...
      std::for_each(first, last, [&freq](char c) {
        auto const it = std::find_if(begin(freq), end(freq),
                                     [c](auto const &p) { return p.key == c; });
        if (it != end(freq)) {
      ...
      std::transform(begin(freq), end(freq), std::back_inserter(ret),
                     [](auto const l) { return std::make_unique<leaf>(l); });
    ...
    static void encoder_rec(node const *cur, std::vector<bool> bits,
                            encoder_t &out) {
...
std::vector<bool> codebook::encode(Iter const first, Iter const last) const {
  ...
  std::for_each(first, last, [&ret, &encoder](char c) {
    auto const it = std::find_if(begin(encoder), end(encoder),
                                 [c](auto const &p) { return p.first == c; });
    if (it != end(encoder)) {
...
std::string codebook::decode(Iter const first, Iter const last) const {
  ...
  // [it', last) = { 1, 1, 0 }
  auto decode_single = [top](Iter it,
                             Iter const last) -> std::pair<Iter, char> {
